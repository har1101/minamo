...
> [!WARNING]
> 実験段階の alpha 版（`0.1.0-alpha.1`）です。API は変わります。
...
- どのツールも、実行中の `ToolCall` を `call` として受け取ります。進捗のイベントを `call.id` で識別するときなどに使います。
...
- **Lambda のクォータ**: 1 つの実行で使えるのは、オペレーション 3,000 個、チェックポイントは累計 100 MB までです。1 つの step の結果（1 回のモデル呼び出しのイベントなど）は 256 KB までです。これを超えると、`@minamojs/lambda-df` は step の名前を示すエラーでその step を失敗させます。長い会話は複数の実行に分けます。リプレイで同じコードが動くように、公開したバージョンかエイリアスを呼び出します。
