...
- `packages/core` (`@minamojs/core`) must stay dependency-free and use Web standard APIs only: no `node:` imports, no `Buffer`, no `AsyncLocalStorage`.
...
- Packages: `@minamojs/core` (core and `@minamojs/core/memory`) and one package per engine (`@minamojs/lambda-df`). All share one version.
